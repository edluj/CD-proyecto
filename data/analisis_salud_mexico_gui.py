...
from analisis_salud_mexico import (
...
class AplicacionAnalisisSalud(tk.Tk):
    def __init__(self) -> None:
        super().__init__()
        self.title("Proyecto Salud Mexico - Interfaz de Regresion Multiple")
        ...
        self.estado = tk.StringVar(value="Cargando contexto del proyecto...")
    ...
    def _crear_interfaz(self) -> None:
        ...
        ttk.Label(
            contenedor,
            text="Proyecto de salud en Mexico - Regresion lineal multiple",
            font=("Segoe UI", 16, "bold"),
        ).pack(anchor="w")
        ttk.Label(
            contenedor,
            text=(
                "La interfaz organiza el flujo en tres pasos: comprobacion de variables, "
    ...
    def _crear_tab_variables(self) -> None:
        ...
        botones = [
            ...
            ("heatmap_general", "Heatmap general", self.abrir_heatmap_general),
    ...
    def _crear_tab_modelo(self) -> None:
        ...
        ttk.Label(
            marco_variables,
            text="Variables independientes finales:",
        ...
        ttk.Button(marco_acciones, text="Entrenar modelo", command=self.ejecutar_modelo).pack(side="left")
    ...
    def _cargar_contexto_inicial(self) -> None:
        ...
        self.estado.set("Contexto cargado. Puedes revisar variables o entrenar el modelo desde la pestaña 2.")
    ...
    def _llenar_resumen_variables(self) -> None:
        ...
        anova = regression["anova_factores_principales"]
        lines = [
            "Comprobacion de variables y evidencias del modelo base",
            "",
            f"Variable dependiente: {regression['variable_dependiente']}",
            f"Variables independientes finales: {', '.join(regression['variables_incluidas_modelo_final'])}",
            "",
        ...
        lines.extend(
            f"- {row['factor']}: F={row['estadistico_f']}, p={row['p_valor']}"
            for row in anova
        )
        lines.extend(
            [
                ...
                "- Importancia de Lasso y Ridge",
                "- Heatmap general y heatmap del modelo",
                "- Diagnostico de regresion y reporte base",
            ]
        )
    ...
    def ejecutar_modelo(self) -> None:
        ...
        self.estado.set("Entrenando modelo y generando resultados...")
        ...
        self.estado.set("Modelo completado. Ya puedes revisar metricas, predicciones y coeficientes en la pestaña 3.")
        self.notebook.select(self.tab_resultados)
        messagebox.showinfo("Modelo finalizado", "El modelo de regresion multiple se entreno correctamente.")
    ...
    def _abrir_artifact(self, key: str) -> None:
        if not self.contexto_base:
            return
        artifacts = self.contexto_base["artifacts"]
